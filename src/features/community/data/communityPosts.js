/** Community posts from travellers eating in Seoul, plus the sub-tab filters. */
export const COMMUNITY_POSTS = [
  {
    id: 'p1',
    kind: 'review',
    locale: 'en',
    author: 'Mia',
    from: 'United States',
    ago: '2h',
    text: 'The kalguksu at Myeongdong Gyoza was the best ₩9,000 I spent in Seoul. Go before noon to skip the line!',
    place: 'Myeongdong Gyoza',
    likes: 48,
    comments: 6,
    photo: true,
    tint: '#FFE3D4',
  },
  {
    id: 'p2',
    kind: 'general',
    locale: 'en',
    author: 'Kenji',
    from: 'Japan',
    ago: '5h',
    text: 'Pro tip: at most BBQ places the staff will grill the meat for you if you look a little lost. Just smile and point!',
    place: 'Gogi House BBQ',
    likes: 122,
    comments: 19,
    photo: false,
    tint: '#FFEFC9',
  },
  {
    id: 'p3',
    kind: 'review',
    locale: 'en',
    author: 'Lena',
    from: 'Germany',
    ago: '1d',
    text: 'Did the Gwangjang market course tonight — bindaetteok and makgeolli is the perfect combo, and way cheaper than I expected.',
    place: 'Gwangjang Bites',
    likes: 87,
    comments: 11,
    photo: true,
    tint: '#E6E9F7',
  },
  {
    id: 'p4',
    kind: 'question',
    locale: 'en',
    author: 'Sofia',
    from: 'Spain',
    ago: '2d',
    text: 'Any veg-friendly spots near Hongdae? Struggling to find good options after 9pm. Recommendations welcome!',
    place: null,
    likes: 14,
    comments: 23,
    photo: false,
    tint: '#E2F1DE',
  },
];

export const COMMUNITY_FILTERS = [
  { key: 'all',      label: 'All',       labelKo: '전체', labelZh: '全部' },
  { key: 'popular',  label: 'Popular',   labelKo: '인기', labelZh: '热门' },
  { key: 'question', label: 'Questions', labelKo: '질문', labelZh: '提问' },
  { key: 'review',   label: 'Reviews',   labelKo: '후기', labelZh: '点评' },
  { key: 'tips',     label: 'Tips',      labelKo: '팁',   labelZh: '贴士' },
  { key: 'food',     label: 'Food',      labelKo: '음식', labelZh: '美食' },
  { key: 'routes',   label: 'Routes',    labelKo: '동선', labelZh: '路线' },
  { key: 'general',  label: 'General',   labelKo: '일반', labelZh: '综合' },
];

/** Apply a sub-tab filter to the posts. */
export function filterPosts(posts, key) {
  if (key === 'popular') return [...posts].sort((a, b) => b.likes - a.likes);
  if (key === 'all') return posts;
  return posts.filter((p) => p.kind === key);
}

/** Filter mock posts by locale for fallback display. */
export function filterPostsByLocale(posts, locale) {
  return posts.filter((p) => p.locale === locale);
}
