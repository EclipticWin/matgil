...
10. bookmarkCount는 카드에 직접 표시하지 않고 Popular 정렬 기준으로만 사용
11. 로딩/에러 상태 문구 추가
12. Common phrases 내부에 All phrases / Popular 2차 탭 제공
13. Popular 내부에 All + 카테고리 필터 제공
```
...
fetchPhrasesByCategory(category)  // mg_phrases WHERE category = ? AND is_active = true ORDER BY sort_order
fetchPopularPhrases({ category = 'all', limit = 10 })
  // mg_phrases WHERE is_active = true AND bookmark_count > 0
  // ORDER BY bookmark_count DESC, id ASC LIMIT limit
  // category !== 'all' 이면 WHERE category = ? 추가
...
- bookmarkCount 숫자 표시는 제거됨. PhraseCard는 하트 채움/비움으로 사용자의 저장 여부만 표시함
- bookmark_count는 Popular 정렬 기준으로만 사용함
...
- `phraseMode` state 추가 (`'all'` | `'popular'`, 초기값 `'all'`)
- `PHRASE_MODE_TABS` 정의 → Common phrases 내부에 All phrases / Popular 2차 탭 렌더링
- `popularCategory` state 추가 (`'all'` 초기값) — All phrases의 `category`와 별도로 관리
- `popularPhrases`, `popularLoading`, `popularFailed` state 추가
- `phraseMode === 'popular'` 진입·카테고리 전환·로그인 상태 변경 시 `fetchPopularPhrases({ category: popularCategory })` 호출 (별도 useEffect, 의존성 배열: `[phraseMode, user, activeTab, popularCategory]`)
- Popular 모드에서 `PhraseCategoryTabs`에 `[{ id: 'all', label: 'All', labelKo: '전체' }, ...categories]` 전달
- Popular > 카테고리 전환 시 `setPopularCategory` 호출 → useEffect 재실행
- `handleBookmark`: `phrases` + `popularPhrases` 양쪽 모두 optimistic update, 북마크 완료 후 `phraseMode === 'popular'`이면 현재 `popularCategory` 기준으로 재조회
- Optimistic update에서 bookmarkCount 증감 로직 제거 (isBookmarked 토글만 수행)
- Popular 빈 상태: `popularCategory === 'all'` → `noPopularPhrases`, 특정 카테고리 → `noPopularPhrasesInCategory`
- Common phrases 내부에 All phrases / Popular 2차 탭 제공
- All phrases 모드에서는 기존 카테고리 탭 기준으로 일반 표현 조회
- Popular 모드에서는 All + 카테고리 탭 기준으로 인기 표현 조회 (bookmark_count 기준 정렬)
...
## 2026-06-29 추가 수정: Popular 카테고리 필터 및 북마크 숫자 UI 제거

### 변경 내용

- 기존에는 PhraseCard 하트 아래 bookmark_count 숫자가 표시되었으나 UI가 지저분하여 제거함
- 북마크 하트는 개인 저장 여부만 표현하도록 변경함 (채움 = 저장됨, 비움 = 미저장)
- Common phrases 내부 2차 탭으로 All phrases / Popular 구조 유지
- Popular 내부에도 All + 기존 카테고리 목록을 표시하도록 변경함
- Popular > All은 전체 인기 표현을 bookmark_count desc 기준으로 표시
- Popular > 특정 카테고리는 해당 카테고리 안에서 인기 표현을 bookmark_count desc 기준으로 표시
- 인기 표현에서도 북마크 추가/해제와 TTS가 기존처럼 동작해야 함
- bookmark_count는 화면 카드에 직접 표시하지 않고 인기순 정렬 기준으로만 사용함

### 수정 파일 (git diff 기준)

| 파일 | 변경 요약 |
|---|---|
| `src/features/phrases/components/PhraseCard.jsx` | 하트 버튼 아래 숫자 span + flex-col 래퍼 제거 |
| `src/features/phrases/services/phraseService.js` | `fetchPopularPhrases({ category, limit })` 신규 추가 |
| `src/pages/PhrasesPage.jsx` | `phraseMode`·`popularCategory` 상태 추가, Popular 전용 useEffect, 카테고리 탭 렌더링, 북마크 재조회 로직 |
| `src/shared/i18n/dictionary.js` | `allPhrases`, `popularPhrases`, `popularAll`, `noPopularPhrases`, `noPopularPhrasesInCategory`, `popularDescription` EN/KO 추가 |

### 테스트 상태 (2026-06-29 기준)

> 2026-06-29 기준 화면을 아주 간단히 확인했을 때 Popular 내부 카테고리 필터는 표시되는 것으로 확인됨.
> 다만 전체 기능 테스트는 아직 완료하지 않음.
> 비로그인/로그인 북마크, Supabase bookmark_count 증감, 새로고침 유지, Voice help 영향 여부는 추가 테스트 필요.
...
`src/shared/i18n/dictionary.js`의 `phrases` 네임스페이스에 총 11개 키가 추가되었습니다 (초기 5개 + 이번 6개).

### 초기 작업 (2026-06-27) 추가 키

```js
// EN / KO
bookmark           // 'Save phrase' / '표현 저장'
bookmarkRemove     // 'Remove saved phrase' / '저장 해제'
bookmarked         // 'Saved' / '저장됨'
loginToBookmark    // 'Log in to save phrases.' / '로그인 후 표현을 저장할 수 있습니다.'
loadError          // 'Could not load phrases.' / '표현을 불러오지 못했습니다.'
```
...
```js
// EN / KO
allPhrases                 // 'All phrases' / '일반 표현'
popularPhrases             // 'Popular' / '인기 표현'
popularAll                 // 'All' / '전체'
noPopularPhrases           // 'No popular phrases yet.' / '아직 인기 표현이 없습니다.'
noPopularPhrasesInCategory // 'No popular phrases in this category yet.' / '이 카테고리에는 아직 인기 표현이 없습니다.'
popularDescription         // 'Most saved phrases' / '많이 저장된 표현'
```

`allPhrases`·`popularPhrases`는 2차 탭 레이블에 사용됩니다. `popularAll`은 Popular 탭 내부 카테고리 탭의 'All' 레이블에 해당하지만, `PhraseCategoryTabs`가 `cat.label`/`cat.labelKo`를 직접 받으므로 컴포넌트 내에서 별도로 `t()`를 호출하지 않습니다.
...
[ ] 8. 하트 아래 bookmark_count 숫자가 표시되지 않는지 확인
[ ] 9. 하트는 저장 여부만 채움/비움으로 표시되는지 확인
[ ] 10. Popular 탭 안에 All / Waiting / Getting seated / Choosing menu ... 카테고리 필터가 표시되는지 확인
[ ] 11. Popular > All에서 전체 인기 표현이 bookmark_count 높은 순으로 표시되는지 확인
[ ] 12. Popular > 특정 카테고리에서 해당 카테고리 인기 표현만 표시되는지 확인
[ ] 13. Voice help 탭 → 기존 음성 인식 흐름 정상 동작 확인
[ ] 14. 카테고리 탭 가로 스크롤 정상 동작 확인 (All phrases 탭 / Popular 탭 모두)
[ ] 15. 한국어/영어 전환 → 카테고리 라벨 및 로그인 안내 문구 정상 전환 확인
[ ] 16. 새로고침 후 북마크 상태 유지 확인
[ ] 17. Popular > 카테고리 선택 후 인기 표현 없을 때 "No popular phrases in this category yet." 표시 확인
```
...
```
1. 화면 테스트 후 버그 수정
2. Popular 탭 UX 세부 조정
3. Popular Top 10 이후 더보기 클릭 시 로그인 유도
4. Bookmarked phrases 탭 추가
5. 표현 검색 기능
6. src/features/phrases/data/phrases.js 삭제 검토 (DB 안정화 후)
7. 관리자 카테고리/표현 관리 기능
