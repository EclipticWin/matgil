...
| 파일 | 상태 | 역할 | 관련 절 |
|---|---|---|---|
| `src/features/community/components/PostCard.jsx` | 이미 커밋됨(`cd217f2`) | 줄바꿈 유지, 5줄 미리보기, 더보기/접기 | §4, §5 |
| `src/features/community/services/communityService.js` | 미커밋(수정) | `fetchPosts()`에 category/limit/offset 파라미터, 서버 필터, 정렬 tiebreaker 추가 | §6, §7, §8 |
| `src/pages/CommunityPage.jsx` | 미커밋(수정) | 페이지네이션 상태, 무한 스크롤 observer, 당겨서 새로고침 연결, 버튼/목록 여백 계산 | §6, §9, §10, §11, §12 |
| `src/features/community/hooks/usePullToRefresh.js` | 미커밋(신규) | 당겨서 새로고침 제스처·상태 머신 | §11 |
| `src/features/community/components/PullToRefreshIndicator.jsx` | 미커밋(신규) | 새로고침 인디케이터 UI | §11, §12 |
| `src/shared/components/AppLayout.jsx` | 미커밋(수정) | 스크롤 컨테이너 ref를 Outlet context로 전달, 하단 내비 높이 CSS 변수 측정 | §11, §12 |
| `src/shared/components/Icon.jsx` | 미커밋(수정) | `RotateCwIcon`(단일 회전 화살표) 추가 | §11 |
| `src/index.css` | 미커밋(수정) | `.matgil-community-scroll`, `.matgil-ptr-content`(-`-dragging`) 클래스, reduced-motion 확장 | §11 |
| `src/shared/i18n/dictionary.js` | 미커밋(수정) | `community.loadMoreError`/`reachedEnd`/`pullToRefresh`/`releaseToRefresh`/`refreshingPosts` 3개 로케일 추가 (`showMore`/`showLess`는 이미 `cd217f2`로 커밋됨) | §10, §11 |
| `docs/67-...md` | 신규(이 문서) | — | — |

### 참고 — 이번 조사에서 확인되지 않아 작업일지에 포함하지 않은 후보

- Supabase RLS/RPC/VIEW 변경 — `communityService.js`의 diff에 `.eq()`/`.order()`/`.range()` 외 새 SQL 호출이 없어, 이번 작업은 순수 클라이언트 쿼리 파라미터 확장이었음을 확인. 별도 SQL/RPC/VIEW는 존재하지 않는다.

---

## 14. 빌드·검사

- `npm run build`: **빌드 확인**, 성공(233 modules transformed, exit 0). CSS 압축 경고 1건(`Expected identifier but found "-"`, `-: T.Z;`)이 발생했으나, 이는 이번 조사 대상 파일과 무관하게 이전 여러 라운드(`docs/64`~`docs/66`)에서도 동일하게 관찰된 기존 경고다.
- `git diff --check`: 통과(exit 0). 출력은 수정된 6개 파일의 "LF will be replaced by CRLF" 경고뿐이며 공백/충돌 마커 오류는 없다.
- 충돌 마커(`<<<<<<<`/`=======`/`>>>>>>>`) 전수 검색: `src/` 전체에서 0건.
- `package.json`의 `scripts`에는 `dev`/`build`/`preview`만 정의되어 있고 별도 `test` 명령이 없음을 확인 — 별도 테스트는 실행하지 않았다.
- 이번 조사 과정에서 코드 파일은 전혀 수정하지 않았다 — 이 작업일지 파일 1개만 신규 작성했다.

---

## 15. 커밋·푸시 결과

이 절은 커밋·푸시 실행 이후 최종 보고에서 실제 값으로 채운다(작업일지 작성 시점에는 아직 커밋 전이므로 계획만 기록).

- 커밋 대상: §13의 미커밋 파일 8개(수정 6 + 신규 2) + 이 작업일지 1개, 총 9개.
- `PostCard.jsx`와 `dictionary.js`의 `showMore`/`showLess` 키는 이미 `cd217f2`로 커밋·푸시되어 있으므로 다시 커밋하지 않는다(다만 `dictionary.js` 자체는 이번 라운드의 `loadMoreError`/`reachedEnd`/`pullToRefresh`/`releaseToRefresh`/`refreshingPosts` 추가분 때문에 다시 수정 상태이며, 이 파일은 이번 커밋에 포함된다).
- 제외 대상: 없음 — 이번 조사에서 확인된 미커밋 변경은 전부 이번 작업 범위(커뮤니티 목록 개선)에 해당하며, 무관한 미완성 파일이나 개인 보관 파일은 `git status`에 존재하지 않았다.

---

## 16. 승인된 한계와 후속 과제

- §12에서 밝힌 대로, 인디케이터·글쓰기 버튼 여백이 실제 화면에서 대수적 계산대로 정확히 대칭으로 보이는지는 이 환경에 브라우저 실행 도구가 없어 확인하지 못했다 — 실기기/개발자도구에서의 확인이 후속 과제로 남는다.
- §11의 pull-to-refresh 전체 흐름(반복 새로고침, 목록 중간 오작동 방지, Samsung Internet/Chrome Android/iOS Safari에서의 브라우저 기본 새로고침 충돌 여부)도 동일한 이유로 실기기 검증이 완료되지 않았다.
- `--matgil-bottom-nav-h` CSS 변수가 실제로 `ResizeObserver`를 통해 정상적으로 갱신되는지도 코드 구조상 타당성만 확인했을 뿐, 실제 렌더링 값으로 재검증하지 않았다.
